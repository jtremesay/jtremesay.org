...
import type { Metadata, ScadaRecord } from "./models"
...
export class ScadaPlayerEngine {
    ctx: CanvasRenderingContext2D
    data: ScadaPlayerData

    constructor(canvas: HTMLCanvasElement, data: ScadaPlayerData) {
    ...
    update(dt: number): void {
        this.data.simulation.update(dt)
        this.data.dashboard.update(this.data.metadata, this.data.simulation)
    }


    render(): void {
        this.data.dashboard.draw(this.ctx, CANVAS_SIZE)
    }
}